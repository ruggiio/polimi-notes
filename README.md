...
2. **dalla trascrizione** (`select_decks`), se il video manca o non mostra slide: per ogni pagina di ogni deck una pertinenza (termini in comune pesati per idf sul corso × quanto sono detti); il deck con media più alta entra sempre, altri (fino a `slides_max_decks`) se hanno media ≥ 35 % del migliore e abbastanza pagine forti. Se però il video c'è e **non mostra nessuna slide del corso** (esercitazione al PC, lavagna, deck non ancora caricato), la trascrizione troverebbe sempre un deck "più vicino" di un'altra lezione: restano solo i deck indicati anche dall'argomento (titolo della lezione nel nome file o nella prima pagina; parole come "intro", "part", "lezione" non contano).
3. **notebook Jupyter** (`.ipynb` nella cartella del corso), se nessun deck è rimasto: per le esercitazioni al PC. Scelti come i deck (argomento + trascrizione, fino a 2), ogni sezione del notebook (fino al titolo `#`–`###` successivo) vale una pagina con markdown e codice; vanno nel prompt per intero, marcati `notebook`, così nomi di funzioni, API e codice negli appunti sono quelli del notebook. Senza video il notebook deve corrispondere all'argomento.
